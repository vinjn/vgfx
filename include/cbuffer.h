...
namespace tr
{
    ...
    */
    template <typename ConstantDataT> class ConstantBuffer
    {
      public:
        ConstantDataT data = {};
    ...
    */
    struct ViewTransformData
    {
        ...
        float3x4
            normal_matrix_world_space;     // float3x3 in HLSL, float3x4 in C++ to pad for alignment
    ...
    class ViewTransformBuffer : public ConstantBuffer<ViewTransformData>
    {
      public:
        ...
        void SetView(const Camera& camera)
        {
            ...
            data.model_view_matrix = data.view_matrix * data.model_matrix;

            data.view_projection_matrix = data.projection_matrix * data.view_matrix;

            data.model_view_projection_matrix =
                data.projection_matrix * data.view_matrix * data.model_matrix;
        ...
        void SetColor(const float3& color) { data.color = float4(color, 0); }

        void SetColor(float r, float g, float b) { data.color = float4(r, g, b, 0); }
    };
    ...
    */
    class BlinnPhongBuffer : public ConstantBuffer<BlinnPhongData>
    {
      public:
        ...
        void SetBaseColor(const float3& base_color) { data.base_color = float4(base_color, 0); }
        ...
        void SetKA(const float3& kA) { data.kA = float4(kA, 0); }

        void SetKD(const float3& kD) { data.kD = float4(kD, 0); }

        void SetKS(const float3& kS) { data.kS = float4(kS, 0); }
    };
