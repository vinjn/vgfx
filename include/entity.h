...
namespace tr
{
    ...
    */
    template <typename LightingParamsT, typename TessParamsT> class EntityT
    {
      public:
        EntityT();
    ...
    template <typename LightingParamsT, typename TessParamsT>
    bool EntityT<LightingParamsT, TessParamsT>::Create(tr_renderer* p_renderer,
                                                       const EntityCreateInfo& create_info)
    {
        // Copy renderer
        ...
        // Descriptor set
        {
            ...
            tr_create_descriptor_set(p_renderer, (uint32_t)descriptors.size(), descriptors.data(),
                                     &m_descriptor_set);
        ...
        // Pipeline
        {
            tr_create_pipeline(m_renderer, m_create_info.shader_program,
                               &m_create_info.vertex_layout, m_descriptor_set,
                               m_create_info.render_target, &m_create_info.pipeline_settings,
                               &m_pipeline);
        ...
        // Constant buffers
        {
            if (has_view_transform)
            {
                ...
                tr_create_uniform_buffer(m_renderer, buffer_size, true, &m_gpu_view_transform);
            ...
            if (has_lighting)
            {
                ...
                tr_create_uniform_buffer(m_renderer, buffer_size, true, &m_gpu_lighting_params);
            ...
            if (has_tess)
            {
                ...
                tr_create_uniform_buffer(m_renderer, buffer_size, true, &m_gpu_tess_params);
    ...
    template <typename LightingParamsT, typename TessParamsT>
    bool EntityT<LightingParamsT, TessParamsT>::SetVertexBuffers(tr_buffer* p_buffer,
    ...
    bool EntityT<LightingParamsT, TessParamsT>::SetVertexBuffers(const tr::Mesh& mesh)
    {
        ...
        tr_create_vertex_buffer(m_renderer, mesh.GetVertexDataSize(), true, mesh.GetVertexStride(),
                                &p_buffer);
    ...
    {
        auto it = std::find_if(std::begin(m_texture_bindings), std::end(m_texture_bindings),
    ...
    void EntityT<LightingParamsT, TessParamsT>::Draw(tr_cmd* p_cmd, uint32_t vertex_count)
    {
        ...
        tr_cmd_bind_vertex_buffers(p_cmd, (uint32_t)m_vertex_buffers.size(),
    ...
    */
    inline tr_shader_program* CreateShaderProgram(tr_renderer* p_renderer,
                                                  ...
                                                  const std::string& ps_entry_point)
    {
        ...
        {
            tr_create_shader_program(p_renderer, (uint32_t)vs.size(), (uint32_t*)vs.data(),
                                     vs_entry_point.c_str(), (uint32_t)ps.size(),
                                     (uint32_t*)ps.data(), ps_entry_point.c_str(),
                                     &p_shader_program);
        }
    ...
    */
    inline tr_shader_program*
    CreateShaderProgram(tr_renderer* p_renderer, const fs::path& vs_file_path,
                        const std::string& vs_entry_point, const fs::path& gs_file_path,
                        const std::string& gs_entry_point, const fs::path& ps_file_path,
                        const std::string& ps_entry_point)
    {
        tr_shader_program* p_shader_program = nullptr;
        ...
        bool has_entry_point =
            !(vs_entry_point.empty() && gs_entry_point.empty() && ps_entry_point.empty());
        if (has_byte_code && has_entry_point)
        {
            tr_create_shader_program_n(
                p_renderer, (uint32_t)vs.size(), (uint32_t*)vs.data(), vs_entry_point.c_str(), 0,
                nullptr, nullptr, 0, nullptr, nullptr, (uint32_t)gs.size(), (uint32_t*)gs.data(),
                gs_entry_point.c_str(), (uint32_t)ps.size(), (uint32_t*)ps.data(),
                ps_entry_point.c_str(), 0, nullptr, nullptr, &p_shader_program);
        }
        return p_shader_program;
    }
    ...
    */
    inline tr_shader_program*
    CreateShaderProgram(tr_renderer* p_renderer, const fs::path& vs_file_path,
                        const std::string& vs_entry_point, const fs::path& hs_file_path,
                        const std::string& hs_entry_point, const fs::path& ds_file_path,
                        const std::string& ds_entry_point, const fs::path& ps_file_path,
                        const std::string& ps_entry_point)
    {
        tr_shader_program* p_shader_program = nullptr;
        ...
        bool has_entry_point = !(vs_entry_point.empty() && hs_entry_point.empty() &&
                                 ds_entry_point.empty() && ps_entry_point.empty());
        if (has_byte_code && has_entry_point)
        {
            tr_create_shader_program_n(
                p_renderer, (uint32_t)vs.size(), (uint32_t*)vs.data(), vs_entry_point.c_str(),
                (uint32_t)hs.size(), (uint32_t*)hs.data(), hs_entry_point.c_str(),
                (uint32_t)ds.size(), (uint32_t*)ds.data(), ds_entry_point.c_str(), 0, nullptr,
                nullptr, (uint32_t)ps.size(), (uint32_t*)ps.data(), ps_entry_point.c_str(), 0,
                nullptr, nullptr, &p_shader_program);
        }
        return p_shader_program;
    }
    ...
    */
    ...
    CreateShaderProgram(tr_renderer* p_renderer, const fs::path& vs_file_path,
                        const std::string& vs_entry_point, const fs::path& hs_file_path,
                        const std::string& hs_entry_point, const fs::path& ds_file_path,
                        const std::string& ds_entry_point, const fs::path& gs_file_path,
                        const std::string& gs_entry_point, const fs::path& ps_file_path,
                        const std::string& ps_entry_point)
    {
        tr_shader_program* p_shader_program = nullptr;
        ...
        bool has_entry_point =
            !(vs_entry_point.empty() && hs_entry_point.empty() && ds_entry_point.empty() &&
              gs_entry_point.empty() && ps_entry_point.empty());
        if (has_byte_code && has_entry_point)
        {
            tr_create_shader_program_n(
                p_renderer, (uint32_t)vs.size(), (uint32_t*)vs.data(), vs_entry_point.c_str(),
                ...
                (uint32_t)ps.size(), (uint32_t*)ps.data(), ps_entry_point.c_str(), 0, nullptr,
                nullptr, &p_shader_program);
        }
        return p_shader_program;
    }
