...
void init_tiny_renderer(GLFWwindow* window)
{
    ...
    unsigned char* image_data = stbi_load((k_asset_dir + "box_panel.jpg").c_str(), &image_width,
                                          &image_height, &image_channels, required_channels);
    ...
    tr_queue_update_texture_uint8(m_renderer->graphics_queue, image_width, image_height,
                                 image_row_stride, image_data, required_channels, m_texture, NULL,
                                 NULL);
    stbi_image_free(image_data);

    tr_create_texture_2d(m_renderer, image_width, image_height, tr_sample_count_1,
                         tr_format_r8g8b8a8_unorm, 1, NULL, false,
                         tr_texture_usage_sampled_image | tr_texture_usage_storage_image,
                         &m_texture_compute_output);

    tr_queue_transition_image(m_renderer->graphics_queue, m_texture_compute_output,
                             tr_texture_usage_undefined, tr_texture_usage_sampled_image);
...
void draw_frame()
{
    ...
    tr_cmd_image_transition(cmd, m_texture_compute_output, tr_texture_usage_sampled_image,
                            tr_texture_usage_storage_image);
    ...
    tr_cmd_image_transition(cmd, m_texture_compute_output, tr_texture_usage_storage_image,
